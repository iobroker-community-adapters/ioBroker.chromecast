...
module.exports = function (_adapter) {
    ...
    var Connection = function (url2play, org_url2play, streamType, callback) {
        ...
        try {
            ...
            that._requestObject.on('error', function (e) {
                adapter.log.error("MediaInformation - ICY connection error for " + that._url2play + ": " + e);
            });
        } catch (e) {
            adapter.log.error("MediaInformation - Cannot connect to " + that._url2play + ": " + e);
        }   
    };
    ...
    Connection.prototype._connected = function (res) {
        adapter.log.info("MediaInformation - Connected to " + that._url2play);
        /* 
    ...
    function getMediaInfoListerner(callerId, url2play, org_url2play, streamType, callback) {
        ...
        adapter.log.debug("Get media info for " + url2play);
    ...
    function closeMediaInfoListerner(callerId) {
        for (var url2play in activeConnections) {
            ...
            adapter.log.debug(callerId + " closing for " + url2play + " - " + JSON.stringify(activeConnections[url2play].callerIDs)); 
            ...
            if (!anyListenerActive) {
                ...
                adapter.log.info("Disconnect " + url2play);
                adapter.log.debug("Last listener was " + callerId);
    ...
    module = {};
    module.getListener   = getMediaInfoListerner;
    module.closeListener = closeMediaInfoListerner;
    return module;
};
