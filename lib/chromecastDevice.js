...
const DELAY_CONNECTION_RETY = 5000; //mseconds
...
var ChromecastDevice = function (adapter, address, name) {
    ...
    function reconnectClient(){
        ...
        if (connectionRetries>MAX_CONECTIONS_RETRIES){
            adapter.log.error(name + " - Max amount of reconnects reached - stay offline");
        } else {
            ...
            if (state && !state.ack) {
                ...
                //Is url2play?
                    ...
                    if (url2play.indexOf("http") == 0) {
                        ...
                        var exported_url = NAMESPACE + '.tts.mp3';
                        //Try to load in a local state
                        try {
                            adapter.setBinaryState(exported_url, fs.readFileSync(url2play));
                        } catch (e) {
                            adapter.log.error('Cannot play file "' + url2play+'": ' + e.toString());
                            return;
                        }
                        //Calculate the exported URL
                        url2play = 'http://192.168.2.68:8082/state/'+exported_url;
