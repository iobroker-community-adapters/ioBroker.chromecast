...
module.exports = function (adapter) {
  ...
  class ChromecastDevice {
    ...
    {
      let that = this;
      if (that._mediaPlayer !== undefined)
      {
        delete that._mediaPlayer;
      }

      adapter.getState(that._states.enabled.name, function (err, state) {
        if (state && (state.val == false)) {                                                                          
          adapter.log.info(that._name + ' - not enabled');                                                            
        }                                                                                                             
        else {

          //Create media player
          that._mediaPlayer = new MediaPlayer(that._connection);
    ...
    _createObjects()
    {
      ...
      //Set some objects
      that._updateMediaPlayerConnection(that._connection);
      that._disconnectedMediaPlayer();
      that._stoppedPlayerStatus();
      
      //Set enabled only if not set already
      adapter.getState(states.enabled.name, function (err, state) {
        if (!state) {
          adapter.setState(states.enabled.name,   {val: true, ack: true});
        }
      });
    ...
    _updateMediaPlayerConnection (connection)
    {
      ...
      adapter.setState(that._states.address.name,   {val: connection.host, ack: true});
      adapter.setState(that._states.port.name,      {val: connection.port, ack: true});
    }
    ...
    _updatePlayerStatus (pStatus, pPreviousStatus)
    {
      ...
      if (status.items &&
        ...
        if (that._mediaPlayer !== undefined) that._mediaPlayer.getStatusPromise();
      }
    ...
    _ioBrokerChange (id, state)
    {
      ...
      //Is enabled
      if (id.indexOf(adapter.namespace + '.' + states.enabled.name) === 0) {
        that._initMediaPlayer();
        adapter.setState(id, {val: state.val,  ack: true});
        return;
      }
      //is adapter disabled?
      else if (that._mediaPlayer === undefined) {
