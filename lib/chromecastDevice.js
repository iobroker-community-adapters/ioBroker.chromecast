...
var ChromecastDevice = function (adapter, address, name, port) {
    ...
    function updatePlayerStatus(pStatus){
        ...
        if (playerState != "STOP" && contentId.indexOf("http") == 0)
            getMediaInfo(contentId, contentId, "LIVE");
    ...
    function getMediaInfo(url2play, org_url2play, streamType, callback){
        //get connection
        connection = MediaInformation.getListener(name, url2play, org_url2play, streamType);

        if (callback) {
            //For the first media event call the callback
            connection.once("media", callback);

            //For all media updates update iobroker state
            connection.on("media", function (media) {   
