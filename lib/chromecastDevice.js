...
var ChromecastDevice = function (adapter, address, name, port) {
    ...
    function updatePlayerStatus(pStatus){
        ...
        setStateIfChanged(states.playerState.name, {val: playerState, ack: true});
    ...
    function getMediaInfo(url2play, org_url2play, streamType, callback){
        ...
        //If discovery is disabled then call callback with constant
        //media info and return
        ...
        try{
            ...
            _getMediaInfo_requestObject.on('error',function(e){
                adapter.log.error(name+" - ICY connection error for "+url2play+": "+e)
            })
        } catch(e){
            adapter.log.error(name+" - Cannot connect to "+url2play+": "+e);
            callback(media);
        }
    }
