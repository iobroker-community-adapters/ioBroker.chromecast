...
module.exports = async function (adapter) {
    ...
    // Detect nettools that is alive
    const instances = await adapter.getObjectViewAsync('system', 'instance', {
        startkey: 'system.adapter.net-tools.',
        endkey: 'system.adapter.net-tools.\u9999'
    });
    ...
    for (let i = 0; i < instances.rows.length; i++) {
        try {
            const ntAlive = await adapter.getForeignStateAsync(`${instances.rows[i].value._id}.alive`);
            if (ntAlive && ntAlive.val) {
                nettoolsInstance = instances.rows[i].value._id.replace('system.adapter.', '');
                break;
            }
        } catch (e) {
            // ignore
    ...
    if (!nettoolsInstance && instances.rows.length) {
        // start first instance
        nettoolsInstance = instances.rows[0].value._id.replace('system.adapter.', '');
        await adapter.extendForeignObjectAsync(`system.adapter.${nettoolsInstance}`, {
            common: {
                enabled: true
            }
        });
    ...
    try {
        const webObj = adapter.getForeignObjectsAsync(`system.adapter.${adapter.config.web}`);
    ...
    class ChromecastDevice {
        ...
        async main() {
            if (this._connection.type === 'Google Cast Group') {
                ...
                let waitForAnswerTimeout = setTimeout(() => {
                    waitForAnswerTimeout = null;
                    adapter.log.error(`No answer from nettools instance ${nettoolsInstance} for "${this._connection.host}"`);
                }, 10000);

                this.id = await adapter.sendToAsync(nettoolsInstance, 'getMac', this._connection.host);
                waitForAnswerTimeout && clearTimeout(waitForAnswerTimeout);
                waitForAnswerTimeout = null;
        ...
        async _updateMediaPlayerConnection(connection) {
            ...
            await adapter.setStateAsync(`${this.id}.port`, {val: connection.port, ack: true});
        }
        ...
        async _updatePlayerStatus(pStatus, pPreviousStatus) {
            ...
            if (status.items &&
                ...
                if (this._mediaPlayer !== undefined) {
                    this._mediaPlayer.getStatusPromise();
                }
            }
