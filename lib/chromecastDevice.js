...
module.exports = function (adapter) {
    ...
    class ChromecastDevice {
        ...
        async main() {
            ...
            this.mac = await adapter.sendToAsync('net-tools.0', 'getMac', this._connection.host);
        ...
        _initMediaPlayer() {
            ...
            adapter.getState(`${this.mac}.enabled`, (err, state) => {
                if (state && !state.val) {
        ...
        _createObjects() {
            ...
            adapter.setObjectNotExists(mac, {
                type: 'device',
                common: {
                    name: name
                },
            ...
            for (const k in channels) {
                if (channels.hasOwnProperty(k)) {
                    adapter.setObjectNotExists(`${mac}.${k}`, {
                        type: 'channel',
                        common: channels[k],
                        native: {}
                    });
            ...
            for (let k in states) {
                if (states.hasOwnProperty(k)) {
                    adapter.setObjectNotExists(this.mac + '.' + k, {
                        type: 'state',
                        common: states[k],
                        native: {}
                    });
            ...
            adapter.getState(`${mac}.enabled`,
                (err, state) => !state && adapter.setState(`${mac}.enabled`, {val: true, ack: true}));

            //Set url2play only if not set already
            adapter.getState(`${mac}.player.url2play`, (err, state) =>
                    !state && adapter.setState(`${mac}.player.url2play`, {val: 'http:/example.org/playme.mp3', ack: true}));
        ...
        _updateMediaPlayerConnection(connection) {
            adapter.setState(`${this.mac}.address`, {val: connection.host, ack: true});
            adapter.setState(`${this.mac}.port`,    {val: connection.port, ack: true});
        }

        _disconnectedMediaPlayer() {
            //Set connected status to false
            adapter.setState(`${this.mac}.status.connected`, {val: false, ack: true});
        }

        _connectedMediaPlayer() {
            //Set playing and connected status to false
            adapter.setState(`${this.mac}.status.connected`, {val: true, ack: true});
        }

        _updateClientStatus(status, previousStatus) {
            //Volume
            adapter.setState(`${this.mac}.status.volume`, {val: this._mediaPlayer.getVolume(), ack: true});
            adapter.setState(`${this.mac}.status.muted`,  {val: this._mediaPlayer.isMuted(), ack: true});

            //Video Chromecast-only
            adapter.setState(`${this.mac}.status.isActiveInput`, {
                val: ('isActiveInput' in status ? status.isActiveInput : true),
                ack: true
            });
            adapter.setState(`${this.mac}.status.isStandBy`, {
            ...
            if (status.hasOwnProperty('applications')) {
                ...
                adapter.setState(`${this.mac}.status.displayName`, {
                    val: ('displayName' in currentApplicationObject ? currentApplicationObject.displayName : ''),
                    ack: true
                });
                adapter.setState(`${this.mac}.status.text`, {
        ...
        _playingPlayerStatus() {
            adapter.setState(`${this.mac}.player.playing`, {val: true, ack: true});
        }

        _stoppedPlayerStatus() {
            adapter.setState(`${this.mac}.player.playing`, {val: false, ack: true});
        }
        ...
        _updatePlayerStatus(pStatus, pPreviousStatus) {
            ...
            this.setStateIfChanged(`${this.mac}.player.playerState`,
                {val: playerState, ack: true},
                cachedPlayerState);

            // paused
            let pause = status.playerState === 'PAUSED';
            this.setStateIfChanged(`${this.mac}.player.paused`,
                {val: pause, ack: true},
            ...
            this.setStateIfChanged(`${this.mac}.player.state`, {val: state, ack: true});

            // currentTime
            this.setStateIfChanged(`${this.mac}.player.currentTime`,
            ...
            if (status.items &&
                ...
                this.setStateIfChanged(`${this.mac}.playlist.list`,
            ...
            //Current Item Id
            this.setStateIfChanged(`${this.mac}.playlist.currentItemId`,
            ...
            this.setStateIfChanged(`${this.mac}.player.repeatMode`,
            ...
            this.setStateIfChanged(`${this.mac}.player.shuffleMode`,
                {val: this.shuffle, ack: true},
                cachedStatus.shuffleMode === 'REPEAT_ALL_AND_SHUFFLE');

            //volume
            this.setStateIfChanged(`${this.mac}.player.playerVolume`,
            ...
            //muted
            this.setStateIfChanged(`${this.mac}.player.playerMuted`,
            ...
            //streamType
            this.setStateIfChanged(`${this.mac}.media.streamType`,
                {val: media.streamType || '', ack: true},
                cachedMedia.streamType || '');

            //duration
            this.setStateIfChanged(`${this.mac}.player.duration`,
                {val: media.duration || 0, ack: true},
                cachedMedia.duration || 0);

            //contentType
            this.setStateIfChanged(`${this.mac}.media.contentType`,
                {val: media.contentType || '', ack: true},
                cachedMedia.contentType || '');

            //contentId
            this.setStateIfChanged(`${this.mac}.player.contentId`,
            ...
            // title
            this.setStateIfChanged(`${this.mac}.player.title`,
                {val: metadata.title || '', ack: true},
                cachedMetadata.title || '');

            // album
            this.setStateIfChanged(`${this.mac}.player.album`,
                {val: metadata.albumName || '', ack: true},
                cachedMetadata.albumName || '');

            // artist
            this.setStateIfChanged(`${this.mac}.player.artist`,
                {val: metadata.artist || '', ack: true},
                cachedMetadata.artist || '');

            // image
            this.setStateIfChanged(`${this.mac}.player.cover`,
            ...
            // broadcast date
            this.setStateIfChanged(`${this.mac}.player.broadcastDate`,
                {val: metadata.broadcastDate || '', ack: true},
                cachedMetadata.broadcastDate || '');
            // Season number
            this.setStateIfChanged(`${this.mac}.player.seasonNumber`,
                {val: metadata.seasonNumber || '', ack: true},
                cachedMetadata.seasonNumber || '');
            // Episode number
            this.setStateIfChanged(`${this.mac}.player.episodeNumber`,
                {val: metadata.episodeNumber || '', ack: true},
                cachedMetadata.episodeNumber || '');
            // Track number
            this.setStateIfChanged(`${this.mac}.player.trackNumber`,
        ...
        _playURL(url2play, org_url2play, streamType) {
            ...
            if (url2play.indexOf('http') !== 0) {
                ...
                let exported_file_state = adapter.namespace + '.' + this.mac + '.exportedMedia';
        ...
        _ioBrokerChange(id, state) {
            ...
            const name = this._name;
            const namespace = adapter.namespace + '.' + this.mac;
            ...
            //Is volume?
            if (id === namespace + '.status.enabled') {
            ...
            //Is muted?
            else if (id === namespace + '.status.muted') {
                if (state.val) {
                    //mute
            ...
            //Is playing?
            else if (id === namespace + '.player.playing') {
                if (state.val) {
                    //Try to play last contentID
                    adapter.getState(namespace + '.player.contentId', (err, state) => {
                        if (state && state.val && state.val.startsWith('http')) {
                            this._playURL(state.val);
                        } else {
                            //Try to play last url2play
                            adapter.getState(namespace + '.player.url2play', (err, state) => {
                                if (state && state.val && state.val.startsWith('http')) {
                                    this._playURL(state.val);
                                } else {
            ...
            // Is pause
            else if (id === namespace + '.player.pause') {
                console.log(id + ' vs. ' + namespace + '.player.pause');
                this._mediaPlayer.pausePromise()
                //ACK written when status update sent by Chromecast
                    .catch(err => adapter.log.error(name + ' - Could not pause: ' + err));
            }
            // Is play
            else if (id === namespace + '.player.play') {
                this._mediaPlayer.playPromise()
                //ACK written when status update sent by Chromecast
                    .catch(err => adapter.log.error(name + ' - Could not play: ' + err));
            }
            // Is stop
            else if (id === namespace + '.player.stop') {
                this._mediaPlayer.stopPromise()
                //ACK written when status update sent by Chromecast
                    .catch(err => adapter.log.error(name + ' - Could not stop: ' + err));
            }
            //Is paused?
            else if (id === namespace + '.player.paused') {
                if (state.val) {
                    this._mediaPlayer.pausePromise()
                    //ACK written when status update sent by Chromecast
                        .catch(err => adapter.log.error(name + ' - Could not pause: ' + err));
            ...
            // Next
            else if (id === namespace + '.player.next') {
            ...
            //Is repeatMode?
            else if (id === namespace + '.player.repeatMode') {
            ...
            //Is repeatAllShuffle?
            else if (id === namespace + '.player.shuffelMode') {
                if (state.val) {
                    state.val = 'REPEAT_ALL_AND_SHUFFLE';
                    this.shuffle = true;
                    this.setStateIfChanged(namespace + '.player.repeatMode', {val: 1, ack: true}, this.repeat);
                } else {
                    this.shuffle = false;
                    ...
                    this.setStateIfChanged(namespace + '.player.repeatMode', {val: state.val, ack: true}, this.repeat);
                }
