...
function getMac(ip) {
    return new Promise((resolve, reject) => arp.getMAC(ip, (err, mac) => (err ? reject(err) : resolve(mac))));
}
...
module.exports = async function (adapter, webPort) {
    ...
    class ChromecastDevice {
        ...
        constructor(connection, isManuallyAddedDevice) {
            ...
            this.main().catch(e => adapter.log.error(`${this._name} - ${e}`));
        } // end of constructor
        ...
        async main() {
            if (this._connection.type === 'Google Cast Group') {
                ...
                if (this._isManuallyAddedDevice) {
                    ...
                    try {
                        ...
                        adapter.log.error(
                            `${this._name} - Error in main() for "${this._connection.host}": ${e.toString()}`,
                        );
                    }
                } else {
                    ...
                    try {
                        ...
                        adapter.log.error(
                            `${this._name} - Cannot get MAC for "${this._connection.host}": ${e.toString()}`,
                        );
                    }
                }
            }
            ...
            if (ChromecastDevice.#knownDeviceIDs.includes(this.id)) {
                adapter.log.error(`${this.id} is not unique, please fix configuration!`);
                return;
            }
            ChromecastDevice.#knownDeviceIDs.push(this.id);
        ...
        async _initMediaPlayer() {
            ...
            if (state && !state.val) {
                ...
                this._mediaPlayer.on(
                    this._mediaPlayer.EVENT_CLIENT_DISCONNECTED,
                    this._disconnectedMediaPlayer.bind(this),
                );
                // player
        ...
        async _updateClientStatus(status /*, previousStatus*/) {
            ...
            // Volume
            if (this._mediaPlayer) {
                await adapter.setStateAsync(`${this.id}.player.volume`, {
                    val: this._mediaPlayer.getVolume(),
                    ack: true,
                });
        ...
        static chromecastRepeat2IobRepeat(chState) {
            return chState === 'REPEAT_ALL' || chState === 'REPEAT_ALL_AND_SHUFFLE'
                ? REPEAT_MODE.ALL
                : chState === 'REPEAT_SINGLE'
                  ? REPEAT_MODE.ONE
                  : REPEAT_MODE.NONE;
        }
        ...
        async _updatePlayerStatus(pStatus, pPreviousStatus) {
            ...
            await this.setStateIfChanged(
                `${this.id}.player.paused`,
                { val: pause, ack: true },
                cachedStatus.playerState === 'PAUSED',
            );

            // state
            let state = ChromecastDevice.chromecastPlayerState2IobState(
                (status.playerState && status.playerState.toLowerCase()) || 'stop',
                pause,
            );
            ...
            // currentTime
            await this.setStateIfChanged(
                `${this.id}.player.currentTime`,
                { val: Math.floor(status.currentTime), ack: true },
                Math.floor(cachedPlayerState.currentTime),
            );

            // Playlist
            if (status.items && status.items.length > 0 && status.items[0] && status.items[0].media) {
                await this.setStateIfChanged(
                    `${this.id}.playlist.list`,
                ...
                this._mediaPlayer &&
                    this._mediaPlayer
                        .getStatusPromise()
                        .catch(err => adapter.log.error(`${this._name}- Cannot get status: ${err}`));
            }

            // Current Item ID
            await this.setStateIfChanged(
                `${this.id}.playlist.currentItemId`,
            ...
            // broadcast date
            await this.setStateIfChanged(
                `${this.id}.metadata.broadcastDate`,
            ...
            // Episode number
            await this.setStateIfChanged(
                `${this.id}.metadata.episodeNumber`,
        ...
        async _playURL(url2play, org_url2play /*, streamType */) {
            ...
            if (!url2play.startsWith('http')) {
                ...
                let exportedFilePath = `chromecast.0.files/${this.id}/exportedMedia.mp3`;
                // Try to load in a local state
                try {
                    try {
                        await adapter.writeFile(adapter.namespace, exportedFilePath, fs.readFileSync(url2play));
                    } catch (err) {
                        adapter.log.error(
                            `${this._name} - Cannot store file "${url2play}" into ${exportedFilePath}: ${err.toString()}`,
                        );
                        return;
                    }
                    // Calculate the exported URL
                    url2play = `http://${adapter.config.webServer}:${webPort}/${exportedFilePath}`;
            ...
            if (youtubeId) {
                ...
                this._launch(Youtube, (err, player) => {
                    if (err) {
                        ...
                        player.load(youtubeId, (err, _status) => {
                            if (err) {
                ...
                this._mediaPlayer &&
                    this._mediaPlayer
                        .playUrlPromise(url2play /*org_url2play, streamType*/)
                        .then(() =>
                            adapter.setStateAsync(`${this.id}.player.url2play`, { val: org_url2play, ack: true }),
                        )
                        .catch(err =>
                            adapter.log.error(`${this._name} - Cannot play file "${url2play}": ${err.toString()}`),
                        );
        ...
        async _ioBrokerChange(id, state) {
            ...
            switch (id) {
                ...
                case 'playlist.jump':
                    this._mediaPlayer
                        .jumpInPlaylistPromise(state.val)
                        ...
                        .setRepeatModePromise(
                            state.val === REPEAT_MODE.ALL || state.val === 'all'
                                ? this.shuffle
                                    ...
                                    : 'REPEAT_ALL'
                                : state.val === REPEAT_MODE.ONE || state.val === 'one'
                                  ? 'REPEAT_SINGLE'
                                  : 'REPEAT_OFF',
                        )
                        // ACK written when status update sent by Chromecast
                ...
                case 'player.shuffelMode':
                    ...
                    this._mediaPlayer
                        .setRepeatModePromise(state.val)
                        // ACK written when status update sent by Chromecast
                ...
                case 'player.announcement':
                    if (state.val && state.val.includes('http')) {
                        this._mediaPlayer
                            .playAnnouncementPromise(state.val)
                            .then(() =>
                                adapter.setStateAsync(`${namespace}.player.announcement`, {
                                    val: state.val,
                                    ack: true,
                                }),
                            )
        ...
        async _networkScannerChange(connection) {
            ...
            if (this._mediaPlayer) {
                ...
                this._initMediaPlayer().catch(e =>
                    adapter.log.error(`${connection.name} - Could not init media player: ${e}`),
                );
            }
