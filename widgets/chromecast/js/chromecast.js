...
function registerForDeviceUpdates($widget, ioBrokerState) {
    ...
    if (vis.states[`${ioBrokerState}.val`] === undefined) {
        ...
        vis.states.bind(`${ioBrokerState}.val`, function (e, newVal) {
...
vis.binds.chromecast = {
        ...
        init: function (widgetID, view, data, style) {
            ...
            if (!$div.length) {
                return setTimeout(function () {
                    vis.binds.chromecast.device.init(widgetID, view, data, style);
                }, 100);
            }
            ...
            for (var devIndex = 1; devIndex <= data.nDevs; devIndex++) {
                ...
                if (validDevId) {
                    ...
                    $select.append(
                        `<option value="${devIndex}" class="chromecastDeviceSelectorValue">${name}</option>`,
                    );
                    ...
                    registerForDeviceUpdates(
                        $device.find('input.chromecastConnected'),
                        data[`oid_connected_${devIndex}`],
                    );
                    ...
                    registerForDeviceUpdates(
                        $device.find('.chromecastDisplayName'),
                        data[`oid_displayName_${devIndex}`],
                    );

                    //Player Description
                    $device.append('<p><span class="chromecastPlayerDescription"/></p>');
                    registerForDeviceUpdates(
                        $device.find('.chromecastPlayerDescription'),
                        data[`oid_statusText_${devIndex}`],
                    );

                    //player state
                    $device.append('<p>Status: <span class="chromecastPlayerState"/></p>');
                    registerForDeviceUpdates(
                        $device.find('.chromecastPlayerState'),
                        data[`oid_playerState_${devIndex}`],
                    );
    ...
    oidChanged: function (widgetID, view, newId, attr, isCss) {
        var mods = [];
        ...
        for (var index = 1; index <= nDevs; index++) {
            ...
            mods += oidDevChanged(widgetID, view, newId, attr, isCss);
        }
        return mods;
    },
};

function oidDevChanged(widgetID, view, newId, attr, isCss) {
    var oid2State = {
    ...
    if (attr.indexOf('oid_dev_') < 0) {
        ...
        return vis.binds.chromecast.oidDevChanged(widgetID, view, newId, attr, isCss);
    }
    ...
    if (validDevId) {
        ...
        for (var oid in oid2State) {
            if (oid2State.hasOwnProperty(oid)) {
                vis.views[view].widgets[widgetID].data[oid + index] = `${validDevId}.${oid2State[oid]}`;
                mods.push(oid + index);
            }
        }
    } else {
        //Reset derived props
        for (var oid_ in oid2State) {
            if (oid2State.hasOwnProperty(oid_)) {
                vis.views[view].widgets[widgetID].data[oid_ + index] = undefined;
                mods.push(oid_ + index);
            }
        }
    }
    return mods;
